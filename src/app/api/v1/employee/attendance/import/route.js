...
async function checkAttendanceThresholds(date) {
  try {
    ...
    const thresholds = await prisma.attendanceThreshold.findMany({ where: { isActive: true } })
      ...
      if (currentTotalCount > threshold.threshold) {
        ...
        const notification = await prisma.notification.create({
          data: {
            ...
            priority: 'high',
            read: false,
            organizationId: threshold.criteria[0].organizationId._id,
            details: {
    ...
    for (const record of attendanceRecords) {
      try {
        ...
        if (existing) {
          failed++;
          errors.push(
            `Duplicate entry: ${employeeCode} (${employeeName || 'Unknown'}) already has attendance on ${new Date(date).toLocaleDateString('en-GB', { timeZone: 'Asia/Kolkata' })}`
          );
          continue;
        }
        ...
        await prisma.attendance.create({ data: data });
        success++;
