...
export async function POST(request) {
    try {
        ...
        const employeeId = exitRequest.employeeId || (exitRequest.modelData && exitRequest.modelData.employee);
        ...
        if (action === "calculate" || !fnfRecord) {
            ...
            const lastWorkingDateStr = exitRequest.modelData?.lastWorkingDate;
            const lastWorkingDate = lastWorkingDateStr ? new Date(lastWorkingDateStr) : new Date();
            ...
            const leaveRecord = leaveRecords.filter(l => l.modelData?.year === currentYear).sort((a,b) => (b.modelData?.month || 0) - (a.modelData?.month || 0))[0];

            const leaveBalance = leaveRecord?.modelData?.annualLeaveBalance?.remaining || 0;
            ...
            const fnfData = {
                ...
                resignationDate: exitRequest.modelData?.resignationDate,
                lastWorkingDate: exitRequest.modelData?.lastWorkingDate,
