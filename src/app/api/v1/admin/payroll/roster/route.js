...
export async function DELETE(req) {
    try {
        ...
        if (id) {
            ...
            // If admin, we should make sure we delete the one matching their organizationId
            if (authUser.role === "admin") {
                filter.shiftData = {
                    path: ['organizationId'],
                    equals: authUser.organizationId
                };
            }
            await prisma.shiftRoster.deleteMany({ where: filter });
        } else {
