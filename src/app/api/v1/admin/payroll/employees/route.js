...
import { buildOrgFilter, normalizeEmployeeRelationIdsArray, flattenModelData } from '@/lib/utils/flatten-model';
...
export async function GET(request) {
  try {
    ...
    // Populate relations manually for Postgres compatibility
    const shiftIds = [...new Set(normalizedEmployees.map(e => e.defaultShift).filter(Boolean))];
    const shifts = await prisma.workingShift.findMany({
      where: { OR: [{ id: { in: shiftIds } }, { mongoId: { in: shiftIds } }] }
    });
    const shiftMap = {};
    shifts.forEach(s => {
      const flattened = flattenModelData(s);
      shiftMap[s.id] = flattened;
      if (s.mongoId) shiftMap[s.mongoId] = flattened;
    });

    const teamIds = [...new Set(normalizedEmployees.map(e => e.teamId).filter(Boolean))];
    const teams = await prisma.team.findMany({
      where: { OR: [{ id: { in: teamIds } }, { mongoId: { in: teamIds } }] }
    });
    const teamMap = {};
    teams.forEach(t => {
      const flattened = flattenModelData(t);
      teamMap[t.id] = flattened;
      if (t.mongoId) teamMap[t.mongoId] = flattened;
    });

    const orgIds = [...new Set(normalizedEmployees.map(e => e.organizationId).filter(Boolean))];
    const orgs = await prisma.organization.findMany({
      where: { OR: [{ id: { in: orgIds } }, { mongoId: { in: orgIds } }] }
    });
    const orgMap = {};
    orgs.forEach(o => {
      const flattened = flattenModelData(o);
      orgMap[o.id] = flattened;
      if (o.mongoId) orgMap[o.mongoId] = flattened;
    });

    const mappedEmployees = normalizedEmployees.map(emp => {
      const mapped = mapEmployeeToMongoose(emp);
      if (emp.defaultShift && shiftMap[emp.defaultShift]) {
        mapped.jobDetails.defaultShift = shiftMap[emp.defaultShift];
      }
      if (emp.teamId && teamMap[emp.teamId]) {
        mapped.jobDetails.teamId = teamMap[emp.teamId];
      }
      if (emp.organizationId && orgMap[emp.organizationId]) {
        mapped.jobDetails.organizationId = orgMap[emp.organizationId];
        mapped.organizationType = orgMap[emp.organizationId].name;
      }
      return mapped;
    });
...
export async function POST(request) {
  try {
    ...
    const employeeData = {
      ...
      bankAccountNumber: encrypt(sDetails.bankAccount?.accountNumber || (isAttendanceOnly ? '000000000' : '')),
      bankName: sDetails.bankAccount?.bankName || (isAttendanceOnly ? 'N/A' : ''),
      ifscCode: encrypt(sDetails.bankAccount?.ifscCode || (isAttendanceOnly ? 'XXXX0000000' : '')),
      branch: sDetails.bankAccount?.branch || '',
      branchAddress: sDetails.bankAccount?.branchAddress || '',
      panNumber: encrypt(sDetails.panNumber || ''),
      aadharNumber: encrypt(sDetails.aadharNumber || ''),
