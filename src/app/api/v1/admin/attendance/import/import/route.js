...
import prisma from "@/lib/db/prisma"; // Updated import
...
async function checkAttendanceThresholds(date) {
  try {
      ...
      include: {
        criteria: {
          include: {
            ...
            employeeCategory: {
              select: { id: true, employeeCategory: true }
            }
          }
        }
      }
    ...
    attendanceRecords.forEach(record => {
      ...
      if (!employee || !employee.jobDetails) return;

      const orgId = employee.jobDetails.organization?.id;
      const employeeType = employee.jobDetails.employeeCategory?.employeeCategory || employee.jobDetails.employeeType || 'Unknown';
      const subType = employee.jobDetails.subType || null; // Assuming subType could be directly on jobDetails or null
      ...
      if (!attendanceCount[key]) {
        attendanceCount[key] = {
          ...
          organizationName: employee.jobDetails.organization?.name || 'Unknown',
          employeeType,
          subType,
          count: 0
        };
    ...
    for (const threshold of thresholds) {
      ...
      for (const criterion of threshold.criteria) {
        if (!criterion.organization) continue; // Use criterion.organization directly due to include

        const orgId = criterion.organization.id;
        const categoryName = criterion.employeeCategory?.employeeCategory || 'Unknown';
        const subType = criterion.subType; // Assuming subType is a field on ThresholdCriterion
      ...
      if (currentTotalCount > threshold.threshold) {
        ...
        const notification = await prisma.notification.create({
          data: {
            ...
            priority: 'high',
            read: false,
            organizationId: threshold.criteria[0].organization?.id, // Use Prisma relation ID
            details: {
              ...
              date: date.toISOString() // Store date as ISO string in JSON
            }
          }
        });
  ...
  try {
    ...
    for (const record of attendanceRecords) {
      try {
        ...
        // 1. FIND EMPLOYEE USING employeeId (employeeCode)
        const employee = await prisma.employee.findFirst({
          where: { employeeId: employeeCode },
          select: {
            id: true, // Need the Prisma ID for linking
            otApplicable: true,
            workingHr: true,
          }
        });
        ...
        recordDate.setHours(12, 0, 0, 0); // Normalize date to avoid timezone issues when checking for duplicates

        const existing = await prisma.attendance.findFirst({
          where: {
            employeeId: employee.id, // Use Prisma employee ID
            date: recordDate,
          },
        });
        ...
        if (existing) {
          failed++;
          errors.push(
            `Duplicate entry: ${employeeCode} (${employeeName || 'Unknown'}) already has attendance on ${recordDate.toLocaleDateString('en-GB', { timeZone: 'Asia/Kolkata' })}`
          );
          continue;
        }
        ...
        const data = {
          employeeId: employee.id, // Use Prisma employee ID as foreign key
          date: recordDate, // Ensure date is a Date object for Prisma
          status,
        ...
        failed++;
        errors.push(`Error processing record for ${record.employeeCode} (${record.employeeName || 'Unknown'}): ${err.message}`);
      }
    }
    ...
    if (success > 0) {
      const uniqueDates = [...new Set(attendanceRecords.map(record => new Date(record.date).toISOString().split('T')[0]))]; // Get unique YYYY-MM-DD strings
