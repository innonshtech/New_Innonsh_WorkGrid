...
async function main() {
  ...
  console.log(`Sending request with token to localhost:3000/api/v1/admin/payroll/v2/runs/${run.id}/action...`);
  
  try {
    const res = await fetch(`http://localhost:3000/api/v1/admin/payroll/v2/runs/${run.id}/action`, {
      method: 'POST',
      headers: {
